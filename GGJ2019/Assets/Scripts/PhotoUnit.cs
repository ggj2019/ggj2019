    ...
    void Update()
    {
        if (isLeaveList)
        {
            if(!leave)
            {
                ...
                {
                    time += Time.deltaTime;
                    sr.color = Color.Lerp(sr.color, new Color(sr.color.r, sr.color.g, sr.color.b, 1), 7 * Time.deltaTime);
                }
            }
        }
        else
        {
            ...
            if (leave)
            {
                ...
                {
                    time -= Time.deltaTime;
                    sr.color = Color.Lerp(sr.color, new Color(sr.color.r, sr.color.g, sr.color.b, 0), 7 * Time.deltaTime);
                }
            }
        }
    }
